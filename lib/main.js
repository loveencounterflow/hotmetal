...
(function() {
  ...
  this.$slice_toplevel_tags = function() {
    return $((function(_this) {
      return function(me, send) {
        return _this.slice_toplevel_tags(me, function(error, slice) {
          if (error != null) {
  ...
  this.$parse = function(settings) {
    ...
    return $((function(_this) {
      return function(html, send) {
        return _this._parse(html, settings, function(error, me) {
          if (error != null) {
            return send.error(error);
          }
          return send(me);
  ...
  this._parse = function(html, settings, handler) {
    ...
    input.pipe(D.HTML.$parse()).pipe(D.HTML.$disperse_texts((_ref = settings['hyphenation']) != null ? _ref : null)).pipe((function(_this) {
      return function() {
        var Z, last_type;
        Z = _this._new_hotml();
        last_type = null;
            ...
            switch (type) {
              ...
              case 'lone-tag':
                if (type === 'text') {
                  ...
                  text = _this._render_open_tag.apply(_this, tail);
                }
                switch (last_type) {
                  case null:
                  case 'close-tag':
              ...
              case 'open-tag':
                switch (last_type) {
                  ...
                  case 'close-tag':
                    ...
                    open_tags.push(_this._render_open_tag.apply(_this, tail));
                    break;
                  case 'open-tag':
                    (CND.last_of(Z))[0].push(_this._render_open_tag.apply(_this, tail));
                    break;
                  default:
              ...
              case 'close-tag':
                switch (last_type) {
                  ...
                  case 'close-tag':
                  case 'open-tag':
                    (CND.last_of(Z))[2].push(_this._render_close_tag.apply(_this, tail));
                    break;
                  default:
  ...
  this.break_lines = function(html, test_line, set_line, handler) {
    var arity;
    switch (arity = arguments.length) {
      case 3:
        handler = set_line;
    ...
    return this.parse(html, (function(_this) {
      return function(error, hotml) {
        var fits, is_first_line, is_first_try, is_last_line, last_slice, last_slice_hotml, lines, slice, slice_hotml, start, stop;
        if (error != null) {
          return handler(error);
        }
        start = 0;
        ...
        while (true) {
          stop += 1;
          is_last_line = (stop > hotml.length) || (stop - start === 0 && stop === hotml.length);
          if (is_last_line) {
            if (last_slice != null) {
              if (set_line != null) {
                set_line(last_slice, is_first_line, is_last_line, last_slice_hotml);
              }
              lines.push(last_slice);
            } else if (slice != null) {
              if (set_line != null) {
                set_line(slice, is_first_line, is_last_line, slice_hotml);
              }
              lines.push(slice);
            }
            return handler(null, lines);
          }
          slice_hotml = _this.slice(hotml, start, stop);
          slice = _this.as_html(slice_hotml);
  ...
  this.$break_lines = function(test_line) {
    return $((function(_this) {
      return function(html, send) {
        return _this.break_lines(html, test_line, null, function(error, lines) {
          return send(lines);
        });
      };
    })(this));
